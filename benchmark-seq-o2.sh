...
BIN_FILE="./sequential"
CFLAGS="-std=c11 -O2 -lm -fopenmp -march=native"

# Compile the application
gcc $SRC_FILE -o $BIN_FILE $CFLAGS

# Execution command
EXEC_CMD="$BIN_FILE"

# Define arrays for sample values of n
...
# Loop over sample n values
for n in "${n_values[@]}"; do
    echo "Running test for n=$n, seed=$SEED"
    
    # Initialize an array to store execution times
    times=()

    # Run the application 10 times for each combination of n
    for i in {1..10}; do
        # Measure the time and suppress the application output
        exec_time=$( { time $EXEC_CMD $n $SEED > /dev/null 2>&1; } 2>&1 )
